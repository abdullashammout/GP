...
const Prescription = ({ navigation }) => {
  ...
  const[data,setData]=useState([{id:'1',title:'prescription 1',createdBy:'sara',medName:'panadol'}]);
  const [modalVisible,setModalVisibile]=useState(false);
  const renderItem = ({ item }) => (
    <TouchableOpacity 
      style={styles.itemContainer} 
      onPress={() => navigation.navigate('presList', { itemId: item.id,itemName:item.createdBy,medName:item.medName})}
    >
      <View>
  ...
  const handleAddItem=()=>{
    setData(prevData=>[...prevData,{id:Math.random().toString(),title:newItem,createdBy:userName,medName:medName}]);
    setNewItem('');
    setUserName('');
    setMedName('');
  ...
  return (
    <View style={styles.container}>
     ...
     <Text onChangeText={text=>setNewItem(text)}>enter doctor name</Text>
      ...
      <Text onChangeText={text=>setMedicineName(text)}>enter medicine name</Text>
      <TextInput style={styles.input} placeholder='med' value={medName} onChangeText={text=>setMedName(text)}/>
